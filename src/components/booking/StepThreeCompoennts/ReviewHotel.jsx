...
const ReviewHotel = ({ BuildItineraryData, setSelectedHotel, switchSummary }) => {
  const hotelData = BuildItineraryData?.[2]?.details ?? []; // safe access
  const [activeIndex, setActiveIndex] = useState(0);          // 0 = first card active
  console.log(hotelData[activeIndex]);
  let filterdata = hotelData[activeIndex]
  // console.log("filter data " , filterdata);



  const handleDeleteHotel = (id) => {
    setSelectedHotel((prev) => prev.filter((f) => f.id !== id));
  ...
  useEffect(() => {
    if (hotelData.length) setActiveIndex(0);
  }, [hotelData]);
          ...
          <div className="mt-2" >
            {
              hotelData.length > 0 ? (
                ...
                <div className="flex flex-wrap gap-5">
                  ...
                  <div className="flex-1 border border-[#EEEEEE] rounded-[26px] shadow-[0px_0px_13px_-3px_#0000001A] px-5 py-4">
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="font-poppins text-xs text-[#58595B]">
                            {hotelData[activeIndex]?.arrivalCity}
                          </span>
                        </div>
                      </div>
                      {/* <MinusSign /> */}
                    ...
                    {/* hotel card */}
                    {/* <SelectedFlightCard crossIcon={false} data={filterdata} className=" -mt-4 w-full shadow-[4px_8px_100px_0px_#1118270F]" /> */}
              ...
              ) : <p className="text-gray-300 text-center w-full font-poppins  " >No Hotel Selected.</p>
            }
          </div>
        )}
    </>
