...
const ReviewTours = ({ BuildItineraryData, setSelectedTour , switchSummary }) => {
  const tourData = BuildItineraryData?.[3]?.details ?? []; // safe access
  const [activeIndex, setActiveIndex] = useState(0);          // 0 = first card active
  console.log(tourData[activeIndex]);
  let filterdata = tourData[activeIndex]
  // console.log("filter data " , filterdata);



  const handleDeleteHotel = (id) => {
    setSelectedTour((prev) => prev.filter((f) => f.id !== id));
  ...
  useEffect(() => {
    if (tourData.length) setActiveIndex(0);
  }, [tourData]);
          ...
          <div className="mt-2" >
            {
                    ...
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="font-poppins text-xs text-[#58595B]">
                            {tourData[activeIndex]?.arrivalCity}
                          </span>
                        </div>
                      </div>
                      {/* <MinusSign /> */}
              ...
              ) : <p className="text-gray-300 text-center w-full font-poppins  " >No Tour Selected.</p>
            }
          </div>
        )}
    </>
